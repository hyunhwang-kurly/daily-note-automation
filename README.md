# 데일리 노트 자동화 (daily-note-automation)
...
| 테스트 | **`node --test`** (내장) | 별도 프레임워크 불필요, 19개 케이스 |
...
├── bin/
│   └── daily-note.js     # CLI 진입점 (launchd가 실행)
├── src/
│   ├── config.js         # 볼트 경로, 루틴(운동/독서), 들여쓰기
│   ├── week.js           # ISO 주차 계산 + 파일 경로 (순수함수)
│   ├── markdown.js       # 섹션 파싱 / 미완료 [ ] 추출 / 이월 일수 누적
│   ├── template.js       # 주간 스켈레톤 생성
│   └── runner.js         # 오케스트레이션 (파일 보장 + 오늘 칸 시드/이월)
├── test/
│   ├── week.test.js      # 주차 규칙 (과거 파일 역검증 포함)
│   ├── markdown.test.js  # 파싱/이월 단위 테스트
│   └── runner.test.js    # 인메모리 IO 통합 테스트
├── launchd/
│   └── install.sh        # 07:00 자동 실행 등록/해제
└── package.json
```
...
**RON = Routine On.** 코드·명령어를 전혀 모르는 사람도 **더블클릭**으로 쓸 수 있게 패키징합니다. Node 런타임이 앱에 포함되어 별도 설치가 필요 없습니다.
