...
### 이월 표시
- 이월된 항목은 **마커 없이 원문 그대로 복사**합니다 (매일 아침 직접 복붙하던 것과 동일).
- 원본 줄의 **들여쓰기(탭)를 보존**하므로 중첩 하위 항목의 계층 구조가 그대로 유지됩니다.
- 기존 노트에 남아있던 레거시 `↪ (Nd)` 접두사는 이월 시 제거하고 순수 텍스트로 옮깁니다.
...
3. **오늘 칸 처리**: Personal에 `운동`·`독서` 시드, Work에 직전 작성일의 미완료 `[ ]`를 마커 없이 원문 그대로(탭 보존) 이월.
...
| 테스트 | **`node --test`** (내장) | 별도 프레임워크 불필요, 48개 케이스 |
...
│   ├── markdown.js       # 섹션 파싱 / 미완료 [ ] 추출(탭 보존) / 이월 마커 제거 (순수)
...
│   ├── obsidian.js       # obsidian:// 딥링크 + https 리다이렉트 URL 생성 (순수)
│   ├── dialog.js         # macOS 네이티브 대화상자 래퍼 (osascript)
│   └── send-mail.applescript  # Mail.app HTML 발송
├── test/                 # node:test (week·markdown·runner·email·obsidian·setup·worklog) 48케이스
├── launchd/
...
│   └── install.sh        # 스킬/훅 설치
├── docs/
│   └── open.html         # 메일 버튼용 https→obsidian:// 리다이렉트 (GitHub Pages)
├── assets/logo.png
...
- **내용**: 주차·요일 헤더, 이월 항목 목록(없으면 "오늘 이월 없음"), 오늘 칸(체크박스 렌더), **`데일리 노트 확인하기` 버튼**.
- **빈 날에도 항상 발송** (변경 없으면 "오늘 이월 없음"으로).
- 메일 발송이 실패해도 **노트 파일 생성은 정상 완료**됩니다 (메일은 best-effort).

### 노트 열기 버튼 — https 리다이렉트 (Gmail 호환)
Gmail 등 웹메일은 보안상 **`obsidian://` 커스텀 스킴 링크를 제거**해 버튼이 클릭되지 않습니다. 이메일 본문은 `<script>`·`onclick` 등 JS가 전부 sanitize 되므로 메일 안에서 직접 여는 방법은 없습니다. 그래서 클릭 가능한 https 링크를 거쳐 `obsidian://`로 넘깁니다.

- **`docs/open.html`** — `?vault=&file=` 쿼리를 받아 `obsidian://open`으로 리다이렉트하는 정적 페이지. **GitHub Pages**(`main` `/docs`)로 호스팅. 볼트/파일을 쿼리로 받는 **범용 페이지**라 모든 사용자가 같은 URL을 공유한다(자동 리다이렉트 + 수동 버튼 + 클립보드 복사 폴백).
- **`obsidian.js`**: `obsidianUri()`(직접 딥링크) + `obsidianWebUri(path, vault, base)`(https 경유). 메일 버튼은 `webBase`가 있으면 후자를, 없으면 전자(Apple Mail 직접 사용 호환)를 사용.
- **설정**: `config.obsidian.webBase`(기본 = 이 저장소의 Pages 주소). `DAILY_NOTE_OBSIDIAN_WEBBASE`로 자체 호스팅 교체 가능.
...
### Obsidian 딥링크 형식
`obsidian://open?vault=<볼트>&file=<상대경로>` 형식. 클릭 시 모바일/PC Obsidian 앱에서 해당 노트가 열립니다. 메일 버튼은 위 "https 리다이렉트"를 거쳐 이 딥링크로 넘기므로 **Gmail 웹에서도 클릭**됩니다(`webBase` 미설정 시에는 이 딥링크를 직접 사용 — Apple Mail 등에서 동작).
...
- `node:test` + `node:assert` — 테스트 48케이스 (별도 프레임워크 없음)
...
### 데이터 / 포맷
- **Markdown** — 노트 본문(정규식 기반 섹션 파서, 체크박스/이월 멱등 마커 `<!-- carried: -->`)
