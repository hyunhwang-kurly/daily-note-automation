-- Mail.app으로 HTML 메일 1통 발송. 인자: subject, address, textBody, htmlBody, fromAddress
-- (셸 이스케이프 회피 위해 argv로 전달)
on run argv
	set theSubject to item 1 of argv
	set theAddress to item 2 of argv
	set theText to item 3 of argv
	set theHtml to item 4 of argv
	set theFrom to item 5 of argv
	...
	tell application "Mail"
		...
		-- 발신 계정 명시 (미지정 시 Outbox에 멈춤). 매칭 계정의 enabled 주소로 설정.
		try
			set sender of newMessage to theFrom
		end try
